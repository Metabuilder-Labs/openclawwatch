...
`ocw` uses TOML configuration. Run `ocw onboard` to generate a starter config, or create one manually.

## Config file discovery

Config is discovered in order (first match wins):

1. `--config` flag or `OCW_CONFIG` env var
2. `ocw.toml` (current directory)
3. `.ocw/config.toml` (current directory)
4. `~/.config/ocw/config.toml` (global)
...
[security]
ingest_secret = "ocw_..."   # generated by ocw onboard
...
Set limits via CLI (`ocw budget --daily 10`), the REST API (`POST /api/v1/budget`), or the web UI.
...
Run `ocw doctor` at any time to verify your configuration:
