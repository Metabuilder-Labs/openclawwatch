"""
Span transport layer. Delivers spans via HTTP POST to the local ocw serve
instance (used when ocw serve is a separate process, or by the TypeScript SDK).
        ...
        for attempt in range(_MAX_RETRIES):
            try:
                ...
                if resp.status_code < 300:
                    ...
                    return True
                logger.warning(
                    "ocw serve returned %d on attempt %d",
                    resp.status_code, attempt + 1,
                )
            except (httpx.ConnectError, httpx.TimeoutException) as exc:
                logger.debug(
                    "ocw serve unreachable (attempt %d/%d): %s",
